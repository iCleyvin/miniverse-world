...
// Use dynamic host for WebSocket so it works behind a reverse proxy
const wsProtocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
const SERVER_WS = `${wsProtocol}//${window.location.host}/ws`;
...
async function main() {
  ...
  if (sceneData?.wanderPoints) props.setWanderPoints(sceneData.wanderPoints);
